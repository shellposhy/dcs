...
var objTitle = [ {
	...
	"fnRender" : function(obj) {
		...
		var result =  '<h3><label class="checkbox inline mt0"><input type="checkbox" id="inlineCheckbox'
				+ obj.aData.id
				+ '" name="idStr'
				+ obj.aData.id
				+ '" value="'
		...
		if(obj.aData.status > 0 && obj.aData.status!=3 ){
			...
			result += '<a class="data_title edit_pop_link" href="'
				+ thisPath
				+ 'data/edit/'
				+ obj.aData.tableId
				+ '/'
		...
		result += '<a  class="padmbt btn floatr none" href="'
				+ thisPath
				+ 'data/info?para='
				+ obj.aData.dataGroups
				+ '"><i class="icon-eye-open" title = "文章预览"></i></a></h3><p class="summary clearfix" >'
				+ sumImg + obj.aData.summary + attach + '</p>';
...
function data_tb() {
	...
	if (titleList != null && titleList != "") {
		$.each(titleList, function(index, field) {
			var temp = {
				"mData" : field
			};
			objTitle.push(temp);
	...
	if(ObjId=='check_success'){
		...
		if($(".dataTables_wrapper").find("table input[type='checkbox']").length > 0){
			...
			$(".dataTables_wrapper").find("table input[type='checkbox']").each(function() {
						if ($(this).attr("checked") && $(this).val() != null
								&& $(this).val().length > 0) {
...
//				"#Article_new_form");
		...
		for ( var i = 0; i < $("#Article_new_form input").length; i++) {
			...
			if (flag) {
				if ($("#Article_new_form input:eq(" + i + ")").hasClass(
						"treeSel")
						|| $("#Article_new_form input:eq(" + i + ")").hasClass(
								"ios_toggle")) {
					continue; // 跳过作为双input的情况
				}
				if ($("#Article_new_form input:eq(" + i + ")").attr("id") == "Imgs") {
					$("#Article_new_form input:eq(" + i + ")").parents(
							".futu_imgs").addClass("beremove");
				} else if ($("#Article_new_form input:eq(" + i + ")")
						.attr("id") == "Sort_Ids") {
					$("#Article_new_form input:eq(" + i + ")").parents(
							".control-group").addClass("beremove");
				} else if ($("#Article_new_form input:eq(" + i + ")")
						.attr("id") == "Data_Status") {
					$("#Article_new_form input:eq(" + i + ")").parents(
							".control-group").addClass("beremove");
				} else if ($("#Article_new_form select:eq(" + i + ")").attr(
						"id") == "Secret_Level") {
					$("#Article_new_form select:eq(" + i + ")").parents(
							".control-group").addClass("beremove");
				} else {
					$("#Article_new_form input:eq(" + i + ")").parents(
							".control-group").addClass("beremove");
